...
import {resiterUserThroughSocial} from '../../store/actions/index';
...
class socialSingup extends Component {
  ...
  render ()
  {
    ...
    this.scroll = ref
        ...
        <View style={{justifyContent:'center',alignContent:'center',alignItems:'center'}}>
          <Text style={{color:'white',fontSize:wp('4.5%'),fontWeight:'bold',paddingVertical:wp('4%')}}>
      ...
      >{this.state.inputs.city.warningText}</Text>
        ...
        disabled={ myNumber && City && country ? false : true}
        onPress={()=>
          {
            let CHeckAge =this.getAge(this.state.DateSelected.substring(5, 7) +'/'+this.state.DateSelected.substring(8, 10) + '/' + this.state.DateSelected.substring(0, 4));
            if(CHeckAge < 18) {
...
const mapsDispatchToProps = dispatch =>{
    return{
        onRegister :(authdata,componentId)=>dispatch(resiterUserThroughSocial(authdata,componentId))
    }
}

export default connect(mapStateToProps,mapsDispatchToProps) (socialSingup);
